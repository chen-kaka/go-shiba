...
func Startup(sessionID string) error {
	...
	config.Hosts = beego.AppConfig.String("mongo.hosts")
	config.Database = beego.AppConfig.String("mongo.database")
	config.UserName = beego.AppConfig.String("mongo.username")
	config.Password = beego.AppConfig.String("mongo.password")
	//if err := envconfig.Process("mgo", &config); err != nil {
	//	log.CompletedError(err, sessionID, "Startup")
	//	return err
	//}
	beego.Info("mongodb config info is: ", config)
